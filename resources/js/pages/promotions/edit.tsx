...
export default function Edit({ promotion }: Props) {
    const [preview, setPreview] = useState<string | null>(promotion.image_url);
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setData('image', file);
        setPreview(file ? URL.createObjectURL(file) : promotion.image_url);
    };

    // Laravel needs a spoofed _method for file uploads sent via PUT/PATCH.
    transform((formData) => ({
        ...formData,
        _method: 'put',
    }));
                    ...
                    <div className="bg-white p-6 shadow-sm sm:rounded-lg">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="flex items-center justify-end gap-3 border-t border-gray-100 pt-4">
                                ...
                                >
                                    {processing ? 'Saving…' : 'Update Promotion'}
                                </button>
                            </div>
                        </form>
                    </div>
